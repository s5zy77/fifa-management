...
export const StadiumMap = ({ zones = [], liveSignals = {}, recommendedGate, recommendedSection, recommendedReset }) => {
  ...
  const getLoadLabel = (load) => {
    if (load < 0.4) return 'Calm';
    if (load < 0.7) return 'Building up';
  ...
  const handleWheel = (e) => {
    ...
    setTransform(prev => ({ ...prev, scale: Math.min(3, Math.max(0.7, prev.scale + delta)) }));
  };
  ...
  const showTooltip = (e, zone) => {
    ...
    const label = isReset ? 'Reset zone' : getLoadLabel(load);
  ...
  const zoomOut = () => setTransform(prev => ({ ...prev, scale: Math.max(0.7, prev.scale - 0.2) }));
  const zoomReset = () => setTransform({ scale: 1, tx: 0, ty: 0 });

  // Pitch constants
  const pitchX = 310, pitchY = 210, pitchW = 280, pitchH = 200;
  const stripes = Array.from({ length: 4 }).map((_, i) => i * 2);

  // Find gate and section coords for route path
  let pathD = null;
    ...
    <section className="card map-card">
      <div className="map-head">
        <div>
          ...
          <p className="sub" style={{ marginBottom: 0 }}>Pinch, scroll, or drag to explore. Tap any zone for details.</p>
        </div>
      ...
      <div className="map-legend">
        ...
        <div className="legend-dot"><span className="dot" style={{ background: 'var(--sky)' }}></span> Reset / sensory room</div>
        <div className="legend-dot"><span className="dot" style={{ background: 'var(--ink)' }}></span> Recommended route</div>
      </div>
    </section>
  );
};
